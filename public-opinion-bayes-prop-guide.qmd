...
This report is a development opportunity for me and a guide for others to better understand Bayesian inference. In it I explore Bayesian proportion tests in R using the {brms} package and data from the Public Opinion of Forestry 2023: UK survey, conducted by YouGov on behalf of Forest Research. The survey asked 11,055 adults (aged 16+) in the UK a range of questions on forestry and forestry-related issues. Here we will explore the results of two questions from the survey: the proportion of respondents who visited woodland in the last few years, and the proportion of respondents that support new woodland creation (afforestation) in response to the threat of climate change.
...
The first thing we need to do is load all the necessary packages for the analysis. Note that {ggGSS} is a personal, in-house package for producing data visualisations that match Government Statistical Service/Government Analysis Function guidance:
...
Note that I use the weighted totals which more accurately reflect the socio-demographic structure of the British population. The `mutate` call rounds all numeric columns to whole numbers (integers) as counts are needed for the models.
...
It is $\pi$ that we are going to estimate. We are going to model $\pi$ using the beta-binomial distribution. The beta-binomial distribution is the binomial distribution in which the probability of success at each of $n$ trials is not fixed but randomly drawn from a beta distribution. It is bound by the the interval [0, 1], which is ideal because $\pi$ is a proportion somewhere between 0 and 1 (i.e., 0% and 100%). (This is set out formulaically later.) The beta distribution is defined by two positive shape parameters: $\alpha$ and $\beta$, where $\alpha$ and $\beta$ correspond to the number of successes (answered "yes") and failures (answered "no"), respectively:
...
#### Figure 5: Posterior distributions of the proportion of <b style="color: white; background-color: #89CFF0;" >men</b> and <b style="color: white; background-color: #F4C2C2;">women</b> that visited woodland in the last few years
...
#### Figure 6: Posterior distribution of the <b style="color: white; background-color: #B78C01;" >difference</b> in proportions
...
#### Table 5: Contingency table showing the proportion of men and women that support afforestation, UK, 2023
...
#### Figure 7: Posterior distributions of the proportion of <b style="color: white; background-color: #89CFF0;" >men</b> and <b style="color: white; background-color: #F4C2C2;">women</b> that support afforestation
...
#### Figure 8: Posterior distribution of the <b style="color: white; background-color: #B78C01;" >difference</b> in proportions
...
> Women are slightly more likely to support tree planting than men in the UK. On average, 84.7% (between 79.0% and 88.4%) of women supported tree planting compared to 82.2% (between 76.9% and 86.6%) of men. There is a 95% posterior probability that the difference between these proportions is between -4.1 and 8.6 percentage points, with a median of 2.4 percentage points. While the difference is small, there is a 86% chance that the difference is not zero. However, support for the hypothesis that support by one gender is much greater than the other is weak.
...
#### Figure 9: Posterior distributions of the proportion of people that <b style="color: white; background-color: #e1ad01;" >agree</b>, <b style="color: white; background-color: #73a580;">neither agree nor disagree</b> and <b style="color: white; background-color: #3e363f;">disagree</b> with new woodland creation
...
q3_beta_binomial_diffs |> 
  ...
  geom_density_pattern(
    ...
    pattern_size = NA, # No border on the stripes
    trim = TRUE,# Trim the ends of the distributions
    linewidth = NA # No border on the distributions
  ) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  scale_x_continuous(expand = expansion(mult = c(0, 0.04)),
                     labels = label_pp) +
