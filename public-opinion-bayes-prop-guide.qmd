...
The {brms} package had a handy function to identify the parameters we can set (i.e., our priors). By passing our equation, data set and family to the `get_prior` function we can see which parameters can be modeled and {brms}'s default values:
...
We get three useful bits of information from the output of the `get_prior` function:

1. The default priors for gender are "flat" (i.e., all values are equally likely). 
2. With `coef = genderFemale` and `coef = genderMale` we can set separate priors for the male/female woodland visitor proportions. I think that would be overkill here. Handily, setting a prior with `class = b` (which refers gender being a population-level effect) will ensure `genderFemale` and `genderMale` have the same prior.
3. The default prior for $\phi$ (the precision parameter) is $\text{Gamma}(0.01, 0.01)$.

Let's set our priors. Note `lb` and `ub` specify the lower and upper bounds, respectively:
...
#### Figure x: Posterior distribution of the proportion of men and women that visited woodland in the last few years
